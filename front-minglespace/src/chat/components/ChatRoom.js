...
const ChatRoom = ({ isFold, wsMembers, workSpaceId, updateRoomParticipantCount, removeRoom }) => {
  ...
  useEffect(() => {
    ...
    const fetchRoomInfo = async () => {
      try {
        ...
        //방 리더인지 확인
        const currentMemberInfo = roomInfo.participants.find(participant => Number(participant.userId) === Number(Repo.getUserId()));
  ...
  const handleInvite = async (addMember) => {
    try {
      ...
      //초대 목록 갱신
      const updatedInviteMembers = inviteMembers.filter((member) => member.wsMemberId !== addMember.wsMemberId);
  ...
  const handleKick = async (kickMember) => {
    try {
      ...
      const data = ChatApi.kickMemberFromRoom(workSpaceId, chatRoomId, kickMember.wsMemberId);

      //참여 멤버 갱신
      const updatedParticipants = chatRoomInfo.participants.filter((member) => member.wsMemberId !== kickMember.wsMemberId);
  ...
  const handleDelegate = async (newLeader) => {
    try {
      const data = await ChatApi.delegateLeader(workSpaceId, chatRoomId, newLeader.wsMemberId);
      ...
      setChatRoomInfo(prev => {
        ...
        return {
          ...prev, participants: updatedParticipants
        };
      });
