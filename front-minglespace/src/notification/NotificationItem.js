...
const NotificationItem = ({ notification }) => {
  ...
  return (
    <div
      ...
      onClick={() =>
        handleConfirmNotification(notification.id, notification.path)
      } //경로 옮기는 함수
    >
      {!notification.read && (
        <span
          ...
          style={{
            backgroundColor:
              dotColorMap[notification.type] || dotColorMap.default,
          }}
        ></span>
      )}
      <span className="notification-msg">{notification.noticeMsg}</span>
      <span className="notification-time">
        {new Date(notification.noticeTime).toLocaleDateString("ko-KR")}
      </span>
    </div>
  );
};
