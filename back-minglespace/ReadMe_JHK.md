...
2. 개발 : 리프레시 토큰 만료시 - 완료
    ...
    - 클라이언트 처리만 남음
3. 개선 : 공용 모달 팝업
4. 개선 : 로그인 로직 정리 : controller -> filter
5. 개선 : AuthController에서 UserController 분리
6. 개선 : OAuth2 로그인시 항시 계정 선택창이 뜰 수 있게
7. 개선 : User Table 정규화
8. 개발 : 스프링, 리액트 통합빌드
    - 스프링에 리액트 빌드해서 넣어야함
    - 이때, url 충돌 이슈 체크해봐야함.
    - /api 넣어야 할 수도 있음
9. 개발 : AWS에 배포
    - github의 actions기능 가능한지 검토
    - aws사용시 비용문제 점검해 봐야함
    - 도메인 사용시 반영해 주어야할 곳 체크해야 함
      - 스프링, 리액트, 각각의 소셜계정에 등록
# 버그
1. 유저정보에서 이미지 갱신시 다른 필드와 같이 변경해야 갱신되는 버그